...
# this script found from:
# https://github.com/acabal/scripts/blob/master/ogg-cover-art
...
# Image file size <32>
# cut --delimiter=' ' --fields=1   # original
# cut -d ' ' -f 1                  # changed for macos - kevin
printf "0: %.8x" $(wc -c "${imagePath}" | cut -d ' ' -f 1) | xxd -r -g0 >> "${imageWithHeader}"
